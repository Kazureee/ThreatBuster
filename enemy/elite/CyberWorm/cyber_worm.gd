...
func spawn():
	var mcyberworm = preload("res://enemy/elite/CyberWorm/mini_cyber_worm.tscn")  # Load the mini CyberWorm scene
	
	for i in range(3):  # Spawn 2 mini CyberWorms
		var worm_instance = mcyberworm.instantiate()  # Create an instance
		worm_instance.global_position = global_position + Vector2(randf_range(-30, 30), randf_range(-30, 30))  # Random offset
		
		get_parent().call_deferred("add_child", worm_instance)  # Safely add to scene

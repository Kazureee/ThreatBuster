...
signal enemy_spawned  # Signal emitted when the Adware spawns
signal enemy_defeated  # Signal emitted when the Adware is defeated
...
func _ready():
	emit_signal("enemy_spawned")  # Emit signal when the Adware spawns
...
func _physics_process(_delta):
	if player:
		...
		# Calculate a perpendicular direction (90 degrees to the left or right)
		var perpendicular_direction = Vector2(-direction_to_player.y, direction_to_player.x)
		
		# Add some randomness to the perpendicular direction
		var random_angle = randf_range(-0.5, 0.5)  # Random angle in radians
		perpendicular_direction = perpendicular_direction.rotated(random_angle)
		
		# Combine the perpendicular direction with the "run away" direction
		var run_away_direction = -direction_to_player  # Reverse the direction to run away
		var combined_direction = (run_away_direction + perpendicular_direction).normalized()
		...
		# Set the velocity to move in the combined direction
...
func take_damage(damage: int, attack_name: String):
	...
	if attack_name.find("ESET") != -1 and is_vulnerable_to_ESET:
		eset_damage = Global.player_ESET
	...
	if malwarebytes_damage > 0:
		show_floating_text(str(int(malwarebytes_damage)), Color(0, 1, 0))  # Green for Malwarebytes
	if eset_damage > 0:
		show_floating_text(str(int(eset_damage)), Color(1, 1, 1))  # White for ESET

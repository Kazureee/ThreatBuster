...
@onready var floating_label = $FloatingLabel  # Reference your pre-created Label
...
func _physics_process(_delta):
	if player:
		...
		if velocity.x < 0:
			$SlimeBody.flip_h = false  # Face left
		elif velocity.x > 0:
			$SlimeBody.flip_h = true  # Face right (default)
...
func take_damage(damage: int, attack_name: String):
	...
	if eset_damage > 0:
		show_floating_text(str(int(eset_damage)), Color(0, 1, 0))  # Green for ESET

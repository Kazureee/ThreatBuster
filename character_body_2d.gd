...
var has_slowed_player = false  # Track if the player has been slowed

func _ready():
	# Apply slowdown to the player when the Ransomware spawns
	add_to_group("ransomware") 
	if player and !has_slowed_player:
		apply_slowdown()
		
		# Show the payment prompt
		var debuff_ui = get_node("/root/Game/UI Layer/RWDebuff")
		if debuff_ui:
			debuff_ui.show_prompt()
		else:
			print("RWDebuff node not found! Check the node path.")
...
func apply_slowdown():
# Slow down the player by 30%
	Global.player_speed *= 0.7
	has_slowed_player = true
	print("Player slowed down by 30%!")
	Global.update_stat()

func remove_slowdown():
	# Restore the player's original speed only if the Ransomware is still active
	if has_slowed_player:
		Global.player_speed /= 0.7
		has_slowed_player = false
...
func take_damage(damage: int, attack_name: String):
	...
	if attack_name.find("Malwarebytes") != -1 and is_vulnerable_to_Malwarebytes:
		malwarebytes_damage = Global.player_Malwarebytes * 1.5
	...
	# If health is less than or equal to 0, handle death
	if health <= 0:
		# Remove the slowdown debuff when the Ransomware is killed
		drop_coin()  # Drop the coin at the last position
		remove_slowdown()
		var debuff_ui = get_node_or_null("/root/Game/UI Layer/RWDebuff")
		if debuff_ui:
			debuff_ui.hide()
			print("RWDebuff UI hidden after Ransomware was destroyed.")
		queue_free()
...
# Function to allow the player to pay life to remove the slowdown
func pay_to_remove_debuff():
	var debuff_cost = 50  # Set the cost to remove slowdown
	
	if Global.player_coins >= debuff_cost:
		Global.remove_coins(debuff_cost)  # 🔹 Use remove_coins to update UI immediately
		remove_slowdown()
		print("Player paid", debuff_cost, "coins to remove the slowdown debuff!")
	else:
		print("Not enough coins to pay! Need", debuff_cost, "coins.")
